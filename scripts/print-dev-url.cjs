...
line(`  ${bold}http://localhost:3000${reset}            ${dim}default, no setup${reset}`);
line(`  ${bold}https://job-assist.dev:3000${reset}      ${dim}branded — needs setup${reset}`);
blank();
line(`Enable the branded URL (one-time, ~30 seconds):`);
line(`  ${bold}brew install mkcert nss${reset}      ${dim}# macOS prereq${reset}`);
line(`  ${bold}npm run setup-domain${reset}         ${dim}# /etc/hosts + TLS cert${reset}`);
console.log(`${bar}${bold}└${'─'.repeat(WIDTH + 2)}┘${reset}`);
console.log('');
